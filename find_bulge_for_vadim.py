...
class PathAnalyzer:
    ...
    def __init__(self, csv_file, output_csv_file):
        ...
        self.new_data = pd.DataFrame(columns=self.data.columns)  # To store the new data including bulges
  ...
  ################################################# math functions ###################################################
    ...
    def calculate_bulge_center(self, start_point, end_point, bulge):
        ...
        angle_between_points = np.arctan2(end_point.northing - start_point.northing, end_point.easting - start_point.easting)
        half_pi = np.pi / 2
        ...
        bulge_radius = self.calculate_bulge_radius(start_point, end_point, bulge)
...
################################################## utility functions ########################################################
    ...
    def find_middle_index(self, start_index, end_index):
        ...
        if num_points_in_bulge % 2 == 1:
            ...
            middle_point = self.create_interpolated_point(self.points[left_middle_index], self.points[right_middle_index])
            self.points.insert(right_middle_index, middle_point)  # Insert the new middle point into the points list
            middle_index = right_middle_index  # Set the middle index to the position of the inserted point
    ...
    def create_interpolated_point(self, point1, point2):
        ...
        new_delta_distance = (point1.delta_distance + point2.delta_distance) / 2
    ...
    def calculate_dynamic_threshold(self, start_index, end_index):
        ...
        avg_heading = np.mean([abs(p.delta_heading) for p in self.points[start_index:end_index + 1]])  # Absolute value for delta heading
        avg_distance = np.mean([p.delta_distance for p in self.points[start_index:end_index + 1]])
        ...
        threshold_unit_constant = BULGE_THRESHOLD + K_DISTANCE * (1 / avg_distance) + K_HEADING * avg_heading
    ...
    def calculate_bulge(self, start_index, middle_index, end_index):
        ...
        start_to_end_dist = np.linalg.norm([end_point.easting - start_point.easting, end_point.northing - start_point.northing])
        start_to_middle_dist = np.linalg.norm([middle_point.easting - start_point.easting, middle_point.northing - start_point.northing])
        middle_to_end_dist = np.linalg.norm([end_point.easting - middle_point.easting, end_point.northing - middle_point.northing])

        if start_to_middle_dist == 0 or middle_to_end_dist == 0:
            return 0  # Avoid division by zero

        # Use cosine rule to find the angle at the middle point
        cos_theta = (start_to_middle_dist**2 + middle_to_end_dist**2 - start_to_end_dist**2) / (2 * start_to_middle_dist * middle_to_end_dist)
    ...
    def calculate_goodness_of_fit(self, start_index, end_index, bulge_value):
        ...
        for i in range(start_index, end_index + 1):
            point = self.points[i]
            distance = self.distance_to_bulge_arc(point, bulge_value, start_index, end_index)
    ...
    def process_potential_curve(self, start_index, end_index):
        ...
        normalized_headings, normalized_distances = self.normalize_segments(start_index, end_index - start_index + 1)
...
##################################################### pre proccessing functions ##############################################
    ...
    def analyze_last_point(self):
        if self.data.iloc[-1]['Type'] == 3:
            final_point = Point(index=len(self.data) - 1,northing=self.data.iloc[-1]['Northing'],easting=self.data.iloc[-1]['Easting'])
    ...
    def analyze_start_points(self):
        ...
        for i in range(len(self.data) - 1):
            if self.data.iloc[i]['Type'] == 2:  # We process only the start points
                ...
                current_point = Point(index=i, northing=self.data.iloc[i]['Northing'], easting=self.data.iloc[i]['Easting'])
                next_point = Point(index=i + 1, northing=self.data.iloc[i + 1]['Northing'], easting=self.data.iloc[i + 1]['Easting'])
                ...
                current_point.delta_heading = self.calculate_delta_heading(current_point, next_point, previous_heading)
                previous_heading = self.calculate_heading(current_point, next_point)  # Update previous_heading for the next iteration
...
############################################################## main functions ######################################################
    ...
    def find_max_curve(self, start_index):
        ...
        max_curve_found = False  # The condition that tells us if we need to keep looking for the max curve or not.
        final_index = len(self.points) - 1  # Adjust the final index to be within the bounds of the points list.
        index_counter = 2  # A bulge will have at least 3 points (start index + 2 points).
        max_points = final_index - start_index  # Limiting the max points based on the total number of points.
        ...
        while index_counter <= max_points:  # Keep running until all points are processed.
            ...
            if end_index > final_index:
                print(f"end_index {end_index} is out of bounds. Stopping curve detection.")
                break

            total_sum = self.process_potential_curve(start_index, end_index)  # Calculate a score for points in between the start and end index.
    ...
    def process_curve_found(self, start_index, end_index, final_index):
        ...
        goodness_of_fit = self.calculate_goodness_of_fit(start_index, end_index, bulge_value)
        print(f"Goodness of fit for optimized bulge from index {start_index} to {end_index}: {goodness_of_fit:.6f}")
    ...
    def process_point(self, start_index):
        ...
        if curve_info:
            ...
            return end_index + 1  # Skip all the points between the start and stop bulge points.
    ...
    def iterate_and_normalize_all_segments(self):
        ...
        while start_index < final_index:
            ...
            start_index = self.process_point(start_index)  # Check if the current index is the start of a bulge
    ...
    def optimize_bulge(self, start_index, middle_index, end_index):
        ...
        initial_bulge = self.calculate_bulge(start_index, middle_index, end_index)  # craeting an initial guess for the bulge
        ...
        for delta in np.linspace(-0.1, 0.1, num=200):  # Adjust the range and step size as needed
            bulge_candidate = initial_bulge + delta
            error = self.calculate_goodness_of_fit(start_index, end_index, bulge_candidate)
            if start_error == None:
                start_error = error
...
############################################### debug functions - no need to copy to js #####################################################################
    def generate_csv_from_points(self):
        ...
        for i in range(len(self.processed_points)):
            ...
            if i > 0:
                ...
                self.new_data = pd.concat([self.new_data, pd.DataFrame(stop_row).T], ignore_index=True)
            ...
            self.new_data = pd.concat([self.new_data, pd.DataFrame(start_row).T], ignore_index=True)
    ...
    def plot_errors_for_each_bulge(self):
        ...
        for i, errors in enumerate(self.bulge_errors, 1):
            ...
            plt.plot(indexes, error_values, marker='o', linestyle='-', color='b')
            plt.xlabel('Index')
            plt.ylabel('Error')
